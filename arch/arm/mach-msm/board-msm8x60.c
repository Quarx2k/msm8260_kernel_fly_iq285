...
#elif defined(CONFIG_KTTECH_SND_CODEC_WM8993)
...
#include <linux/qt602240_kttech.h>
#endif
#ifdef CONFIG_KTTECH_TOUCH_QT602240_V3
#include <linux/qt602240_kttech_v3.h>
#endif
...
#ifndef CONFIG_MACH_KTTECH  //sleep current
/*
...
#else // KT Tech Remove EHCI
static int pmic_id_notif_supported = 0;
...
static char *usb_functions_default_adb_ftm[] = {
	"diag",
	"adb",
	"modem",
	"nmea",
...
#ifndef CONFIG_MACH_KTTECH // RNDIS Debugging
	{
#ifdef CONFIG_MACH_KTTECH
		.product_id	= 0x3502,
#else
...
#ifdef CONFIG_MACH_KTTECH
	.vendorID	= 0x2116,
	.vendorDescr	= "KT Tech Incorporated",
#else
	.vendorID	= 0x05C6,
...
static struct android_usb_platform_data android_usb_pdata = {
	...
	.version	= 0x0100,
#ifdef CONFIG_MACH_KTTECH
	.product_name		= "KT Tech HSUSB Device",
	.manufacturer_name	= "KT Tech Incorporated",
#else
...
#elif defined(CONFIG_KTTECH_MT9M113) || defined(CONFIG_KTTECH_MT9E013) //jykim boba
static struct msm_camera_sensor_flash_src msm_flash_src = {
	.flash_sr_type = MSM_CAMERA_FLASH_SRC_PMIC,
	._fsrc.pmic_src.num_of_src = 2,
#ifdef CONFIG_MACH_KTTECH 
		/* Camera flash LED current change to min 100mA -> 180mA*/
	._fsrc.pmic_src.low_current  = 180,
#else
	._fsrc.pmic_src.low_current  = 100,
#endif
	._fsrc.pmic_src.high_current = 300,
...
#endif
#endif
#if defined(CONFIG_KTTECH_MT9M113) || defined(CONFIG_KTTECH_MT9E013) //jykim
#define CAM_MCLK         32
#define CAM_1M_STANBY_N  97
#define CAM_1M_RST_N     98
#define CAM_8M_RST_N    100
...
static uint32_t camera_on_gpio_table[] = {
	...
	GPIO_CFG(CAM_1M_STANBY_N,0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 
	GPIO_CFG(CAM_1M_RST_N,0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 
	GPIO_CFG(CAM_8M_RST_N,0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 
};
#else
int msm_cam_gpio_tbl[] = {
	32,/*CAMIF_MCLK*/
...
#if !defined(CONFIG_KTTECH_MT9M113) && !defined(CONFIG_KTTECH_MT9E013) // jykim : GPIO_EXT_CAMIF_PWR_EN is used to enable the Vdd of the camera on SURF
	rc = gpio_request(GPIO_EXT_CAMIF_PWR_EN, "CAM_EN");
	if (rc < 0) {
		config_gpio_table(MSM_CAM_OFF);
		printk(KERN_ERR "%s: CAMSENSOR gpio %d request"
...
static void config_camera_off_gpios(void)
{
	...
	gpio_free(GPIO_EXT_CAMIF_PWR_EN);
#else
	gpio_set_value_cansleep(CAM_1M_STANBY_N, 0);
	gpio_set_value_cansleep(CAM_1M_RST_N, 0);
	gpio_set_value_cansleep(CAM_8M_RST_N, 0);
#endif

	config_gpio_table(MSM_CAM_OFF);
}

#ifdef CONFIG_KTTECH_MT9M113
static int config_camera_on_gpios_mt9m113(void)
{
	int rc = 0;
	...
	gpio_set_value_cansleep(CAM_8M_RST_N, 1);
	mdelay(2);

	gpio_set_value_cansleep(CAM_1M_RST_N, 1);
	return rc;
}
#endif
...
#ifdef CONFIG_KTTECH_MT9M113 // jykim 1.3M SOC Camera
...
struct msm_camera_device_platform_data msm_camera_device_data_mt9m113 = {
	.camera_gpio_on  = config_camera_on_gpios_mt9m113, 
	.camera_gpio_off = config_camera_off_gpios,
	.ioext.csiphy = 0x04900000,
	.ioext.csisz  = 0x00000400,
	.ioext.csiirq = CSI_1_IRQ,
...
static struct msm_camera_sensor_info msm_camera_sensor_mt9m113_data = {
	.sensor_name	= "mt9m113",
	.sensor_reset	= CAM_1M_RST_N, /*CAM_IM_RST_N*/ 
	.sensor_pwd		= 0,
	.vcm_pwd		= 0,
	.vcm_enable		= 0,
...
#ifdef CONFIG_KTTECH_MT9E013  // jykim 8M Bayer Camera
...
static struct msm_camera_sensor_info msm_camera_sensor_mt9e013_data = {
	.sensor_name	= "mt9e013",
	.sensor_reset	= CAM_8M_RST_N, /*CAM_8M_RST_N*/
	.sensor_pwd		= 0,
	.vcm_pwd		= 0,
	.vcm_enable		= 0,
	.pdata			= &msm_camera_device_data,
...
static struct i2c_board_info msm_camera_boardinfo[] __initdata = {
	...
	#endif
	#ifdef CONFIG_KTTECH_MT9M113 // jykim
	{
		I2C_BOARD_INFO("mt9m113", 0x78),
	},
	#endif
	#ifdef CONFIG_KTTECH_MT9E013
	{
...
#ifdef CONFIG_KTTECH_TOUCH_MCS7000
...
#endif /*CONFIG_KTTECH_BATTERY_GAUGE_MAXIM*/
#endif
...
static int msm_fb_detect_panel(const char *name)
{
	...
	{
		if(!strcmp(name, "mipi_video_dummy_wvga"))
			return 0;
	...
	{
		if(!strcmp(name, "mipi_video_novatek_qhd"))
			return 0;
		pr_warning("%s: not supported '%s'", __func__, name);
		return -ENODEV;
	}
	if (!strcmp(name, "mipi_cmd_novatek_qhd")) // By JhoonKim : "mipi_cmd_novatek_qhd"
		return 0;
#else
	if (machine_is_msm8x60_fluid()) {
...
#endif //ifdef CONFIG_MACH_KTTECH
}
...
#if 0 // KT Tech !defined(CONFIG_KTTECH_TOUCH_QT602240) && !defined(CONFIG_KTTECH_TOUCH_MCS7000)
...
static struct rpm_vreg_pdata rpm_vreg_init_pdata[RPM_VREG_ID_MAX] = {
	RPM_VREG_INIT_LDO_PF(PM8058_L0,  0, 0, 0, 1200000, 1200000, LDO150HMIN,
		RPM_VREG_PIN_CTRL_NONE, RPM_VREG_PIN_FN_SLEEP_B),
	RPM_VREG_INIT_LDO(PM8058_L1,  0, 0, 0, 1200000, 1200000, LDO300HMIN, 0),
	RPM_VREG_INIT_LDO(PM8058_L2,  0, 0, 0, 1800000, 2700000, LDO300HMIN, 0),
	RPM_VREG_INIT_LDO(PM8058_L3,  0, 0, 0, 1800000, 1800000, LDO150HMIN, 0),
	RPM_VREG_INIT_LDO(PM8058_L4,  0, 0, 0, 2850000, 2850000,  LDO50HMIN, 0),
...
#if defined(CONFIG_KTTECH_MT9E013) || defined(CONFIG_KTTECH_MT9E013) 
	RPM_VREG_INIT_LDO(PM8058_L8,  0, 1, 0, 2800000, 2800000, LDO300HMIN, 0),
#else
	RPM_VREG_INIT_LDO(PM8058_L8,  0, 1, 0, 2900000, 3050000, LDO300HMIN, 0),
#endif
#if defined(CONFIG_KTTECH_MT9M113) || defined(CONFIG_KTTECH_MT9E013) 
	RPM_VREG_INIT_LDO(PM8058_L9,  0, 1, 0, 1800000, 1800000, LDO300HMIN, 0),
#else
	RPM_VREG_INIT_LDO(PM8058_L9,  0, 1, 0, 1800000, 1800000, LDO300HMIN, 0),
#endif
	RPM_VREG_INIT_LDO(PM8058_L10, 0, 0, 0, 2600000, 2600000, LDO300HMIN, 0),
#if defined(CONFIG_KTTECH_SOUND_WM8993) || defined(CONFIG_KTTECH_SND_CODEC_WM8993)
	RPM_VREG_INIT_LDO(PM8058_L11, 0, 0, 0, 3000000, 3000000, LDO150HMIN, 0),
#elif defined(CONFIG_KTTECH_SOUND_YDA165)
	RPM_VREG_INIT_LDO(PM8058_L11, 0, 0, 0, 2600000, 2600000, LDO150HMIN, 0),
#else
	RPM_VREG_INIT_LDO(PM8058_L11, 0, 0, 0, 1500000, 1500000, LDO150HMIN, 0),
#endif
#if defined(CONFIG_KTTECH_MT9E013)
...
#endif
	RPM_VREG_INIT_LDO(PM8058_L13, 0, 0, 0, 2050000, 2050000, LDO300HMIN, 0),
	RPM_VREG_INIT_LDO(PM8058_L14, 0, 0, 0, 2850000, 2850000, LDO300HMIN, 0), 
	RPM_VREG_INIT_LDO(PM8058_L15, 0, 0, 0, 2850000, 2850000, LDO300HMIN, 0), 
	RPM_VREG_INIT_LDO(PM8058_L16, 1, 0, 1, 1800000, 1800000, LDO300HMIN, 0),
#ifdef CONFIG_MACH_KTTECH	
	RPM_VREG_INIT_LDO(PM8058_L17, 0, 0, 0, 2700000, 2700000, LDO150HMIN, 0),
#else
	RPM_VREG_INIT_LDO(PM8058_L17, 0, 0, 0, 2600000, 2600000, LDO150HMIN, 0),
#endif
	RPM_VREG_INIT_LDO(PM8058_L18, 0, 0, 1, 2200000, 2200000, LDO150HMIN, 0),
#ifdef CONFIG_KTTECH_WIFI_BCM4329
    RPM_VREG_INIT_LDO(PM8058_L19, 0, 0, 0, 1800000, 1800000, LDO150HMIN, 0),
#else
	RPM_VREG_INIT_LDO(PM8058_L19, 0, 0, 0, 2500000, 2500000, LDO150HMIN, 0),
#endif	
	RPM_VREG_INIT_LDO_PF(PM8058_L20, 0, 0, 0, 1800000, 1800000, LDO150HMIN,
		RPM_VREG_PIN_CTRL_NONE, RPM_VREG_PIN_FN_SLEEP_B),
	RPM_VREG_INIT_LDO_PF(PM8058_L21, 1, 0, 0, 1200000, 1200000, LDO150HMIN,
		RPM_VREG_PIN_CTRL_NONE, RPM_VREG_PIN_FN_SLEEP_B),
	...
	RPM_VREG_INIT_LDO(PM8058_L24, 0, 0, 0, 1200000, 1200000, LDO150HMIN, 0),
	RPM_VREG_INIT_LDO(PM8058_L25, 0, 0, 0, 1200000, 1200000, LDO150HMIN, 0),

	RPM_VREG_INIT_SMPS(PM8058_S0, 0, 0, 1,  500000, 1250000,  SMPS_HMIN, 0,
		RPM_VREG_FREQ_1p60),
	RPM_VREG_INIT_SMPS(PM8058_S1, 0, 0, 1,  500000, 1250000,  SMPS_HMIN, 0,
		RPM_VREG_FREQ_1p60),
	RPM_VREG_INIT_SMPS(PM8058_S2, 0, 0, 1, 1200000, 1400000,  SMPS_HMIN,
		RPM_VREG_PIN_CTRL_A0, RPM_VREG_FREQ_1p60),
	RPM_VREG_INIT_SMPS(PM8058_S3, 1, 0, 0, 1800000, 1800000,  SMPS_HMIN, 0,
		RPM_VREG_FREQ_1p60),
	RPM_VREG_INIT_SMPS(PM8058_S4, 1, 0, 0, 2200000, 2200000,  SMPS_HMIN, 0,
		RPM_VREG_FREQ_1p60),

	RPM_VREG_INIT_VS(PM8058_LVS0, 0, 0, 0,				 0),
	RPM_VREG_INIT_VS(PM8058_LVS1, 0, 0, 0,				 0),

	RPM_VREG_INIT_NCP(PM8058_NCP, 0, 0, 0, 1800000, 1800000,	 0),

	RPM_VREG_INIT_LDO(PM8901_L0,  0, 0, 0, 1200000, 1200000, LDO300HMIN,
		RPM_VREG_PIN_CTRL_A0),
#ifdef CONFIG_KTTECH_MOTOR_MDH1_VIBETONZ
	RPM_VREG_INIT_LDO(PM8901_L1,  0, 0, 0, 3300000, 3300000, LDO300HMIN, 0),
#else
	RPM_VREG_INIT_LDO(PM8901_L1,  0, 0, 0, 3300000, 3300000, LDO300HMIN, 0),
#endif
	RPM_VREG_INIT_LDO(PM8901_L2,  0, 0, 0, 2850000, 3300000, LDO300HMIN, 0),
	RPM_VREG_INIT_LDO(PM8901_L3,  0, 0, 0, 3000000, 3300000, LDO300HMIN, 0),
#if defined(CONFIG_KTTECH_TOUCH_QT602240) || defined(CONFIG_KTTECH_TOUCH_QT602240_V3)
	RPM_VREG_INIT_LDO(PM8901_L4,  0, 0, 0, 2700000, 2700000, LDO300HMIN, 0),
#elif defined(CONFIG_KTTECH_TOUCH_MCS7000)
	RPM_VREG_INIT_LDO(PM8901_L4,  0, 0, 0, 2800000, 2800000, LDO300HMIN, 0),
#else
	RPM_VREG_INIT_LDO(PM8901_L4,  0, 0, 0, 2600000, 2600000, LDO300HMIN, 0),
#endif
	...
	RPM_VREG_INIT_SMPS(PM8901_S2, 0, 0, 0, 1300000, 1300000,   FTS_HMIN, 0,
		RPM_VREG_FREQ_1p60),
	RPM_VREG_INIT_SMPS(PM8901_S3, 0, 0, 0, 1100000, 1100000,   FTS_HMIN, 0,
		RPM_VREG_FREQ_1p60),
	RPM_VREG_INIT_SMPS(PM8901_S4, 0, 0, 0, 1225000, 1225000,   FTS_HMIN,
		RPM_VREG_PIN_CTRL_A0, RPM_VREG_FREQ_1p60),

	RPM_VREG_INIT_VS(PM8901_LVS0, 1, 0, 0,				 0),
	RPM_VREG_INIT_VS(PM8901_LVS1, 0, 0, 0,				 0),
...
#elif defined(CONFIG_KTTECH_WIFI_BCM4329)     //KTTECH_BT
static int bluetooth_power(int);
static struct platform_device msm_bt_power_device = {
	.name	 = "bt_power",
	.id	 = -1,
...
#endif	
#ifndef CONFIG_MACH_KTTECH // Remove Temp Tmroh !defined(CONFIG_KTTECH_TOUCH_QT602240) && !defined(CONFIG_KTTECH_TOUCH_MCS7000)
	&msm_gsbi3_qup_i2c_device,
#endif	
	&msm_gsbi4_qup_i2c_device,
	&msm_gsbi7_qup_i2c_device,
...
#else
        // By JhoonKim : modified for O3 Vol UP/Down
...
static struct resource resources_othc_2[] = {
	...
	{
		.start = MSM_GPIO_TO_INT(46),
		.end   = MSM_GPIO_TO_INT(46),
		.flags = IORESOURCE_IRQ,
	},
	{
		.start = MSM_GPIO_TO_INT(99),
		.end   = MSM_GPIO_TO_INT(99),
		.flags = IORESOURCE_IRQ,
	},
#endif
	{
...
static struct regulator *vreg_timpani_1; //8058_l24  : dig_1.2v
static struct regulator *vreg_timpani_2; //8058_s3   : at_io_1.8v

static unsigned int msm_timpani_setup_power(void)
{
	int rc;

#if defined(CONFIG_KTTECH_SND_CODEC_WM8993)
	vreg_timpani_1 = regulator_get(NULL, "8058_l11");
#else
	vreg_timpani_1 = regulator_get(NULL, "8058_l24");
...
#if defined(CONFIG_KTTECH_SND_CODEC_WM8993)
	vreg_timpani_2 = regulator_get(NULL, "8901_lvs2");
#else
...
#if defined(CONFIG_KTTECH_SND_CODEC_WM8993)
	rc = regulator_set_voltage(vreg_timpani_1, 3000000, 3000000);
#else
...
#ifdef CONFIG_KTTECH_SND_CODEC_WM8993
static struct regulator *vreg_wm8993_cdc_apwr1;
static struct regulator *vreg_wm8993_cdc_apwr2;
#else
...
#if defined(CONFIG_KTTECH_SND_CODEC_WM8993)
	if (!vreg_wm8993_cdc_apwr1) {
		vreg_wm8993_cdc_apwr1 = regulator_get(NULL, "8058_l11");
		if (IS_ERR(vreg_wm8993_cdc_apwr1)) {
			pr_err("%s: vreg_get failed (%ld)\n", __func__, PTR_ERR(vreg_wm8993_cdc_apwr1));
			rc = PTR_ERR(vreg_wm8993_cdc_apwr1);
			return rc;
		}
	}

	if (!vreg_wm8993_cdc_apwr2) {
		vreg_wm8993_cdc_apwr2 = regulator_get(NULL, "8901_lvs2");
		if (IS_ERR(vreg_wm8993_cdc_apwr2)) {
			pr_err("%s: vreg_get failed (%ld)\n", __func__, PTR_ERR(vreg_wm8993_cdc_apwr2));
			rc = PTR_ERR(vreg_wm8993_cdc_apwr2);
			return rc;
		}
	}

	if (vreg_on) {
		rc = regulator_set_voltage(vreg_wm8993_cdc_apwr1, 3000000, 3000000);
		if (rc) {
			pr_err("%s: unable to set 8058_l12 voltage to 2.2 V\n", __func__);
			goto vreg_fail;
		}

		rc = regulator_enable(vreg_wm8993_cdc_apwr1);
		if (rc) {
			pr_err("%s: vreg_enable failed %d\n", __func__, rc);
			goto vreg_fail;
		}

		rc = regulator_enable(vreg_wm8993_cdc_apwr2);
		if (rc) {
			pr_err("%s: vreg_enable failed %d\n", __func__, rc);
			goto vreg_fail;
		}
	} else {
		rc = regulator_disable(vreg_wm8993_cdc_apwr1);
		if (rc) {
			pr_err("%s: vreg_disable failed %d\n", __func__, rc);
			goto vreg_fail;
		}

		rc = regulator_disable(vreg_wm8993_cdc_apwr2);
		if (rc) {
			pr_err("%s: vreg_disable failed %d\n", __func__, rc);
			goto vreg_fail;
		}
...
vreg_fail:
	regulator_put(vreg_wm8993_cdc_apwr1);
	vreg_wm8993_cdc_apwr1 = NULL;
	regulator_put(vreg_wm8993_cdc_apwr2);
	vreg_wm8993_cdc_apwr2 = NULL;
	return rc;
#else
...
#if defined(CONFIG_KTTECH_SND_CODEC_WM8993)
#define TIMPANI_I2C_SLAVE_ADDR  (0x34>>1) /* fixed address */
#else
#define TIMPANI_I2C_SLAVE_ADDR	0xD
#endif /*CONFIG_KTTECH_SND_CODEC_WM8993*/

#if defined(CONFIG_KTTECH_SND_CODEC_WM8993) && defined(CONFIG_KTTECH_TOUCH_MCS7000) // only WS1
static struct i2c_board_info msm_i2c_gsbi8_timpani_info[] = {
	{
		I2C_BOARD_INFO("timpani", TIMPANI_I2C_SLAVE_ADDR),
		.platform_data = &timpani_pdata,
	},
};
#else //#elif defined(CONFIG_KTTECH_SND_CODEC_WM8993) && defined(KTTECH_TOUCH_QT602240)
...
static struct regulator *vreg_yda165_sw;

static int msm_yda165_setup_power(struct device *dev)
{
	int rc = 0;
	const char *msm_yda165_regulator = "8058_l11";

	vreg_yda165 = regulator_get(dev, msm_yda165_regulator);
	if (IS_ERR(vreg_yda165)) {
		rc = PTR_ERR(vreg_yda165);
		dev_err(dev, "%s: regulator_get %s = %d\n", __func__,
			msm_yda165_regulator, rc);
	}
	...
	if (!rc)
		vreg_yda165_sw = regulator_get(NULL, "8901_mvs0");
	else {
		dev_err(dev, "%s: regulator_get %s = %d\n", __func__,
			msm_yda165_regulator, rc);
		goto unget;
	...
	if (IS_ERR(vreg_yda165_sw)) {
		rc = PTR_ERR(vreg_yda165_sw);
		dev_err(dev, "%s: regulator_get %s = %d\n", __func__,
			msm_yda165_regulator, rc);
	}

	if (!rc)
		rc = regulator_enable(vreg_yda165_sw);
	else {
		dev_err(dev, "%s: regulator_enable %s = %d\n", __func__,
			msm_yda165_regulator, rc);
		goto unenable;
...
{
	regulator_disable(vreg_yda165_sw);
	regulator_put(vreg_yda165_sw);
...
static struct i2c_board_info pm8901_boardinfo[] __initdata = {
	...
	&& !defined(CONFIG_KTTECH_WIFI_BCM4329)
...
static struct i2c_registry msm8x60_i2c_devices[] __initdata = {
...
#if defined(CONFIG_KTTECH_SND_CODEC_WM8993) && defined(CONFIG_KTTECH_TOUCH_MCS7000) // only WS1
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,		
		MSM_GSBI8_QUP_I2C_BUS_ID,	
		msm_i2c_gsbi8_timpani_info,
		ARRAY_SIZE(msm_i2c_gsbi8_timpani_info),
	},
#else //#elif defined(CONFIG_KTTECH_SND_CODEC_WM8993) && defined(KTTECH_TOUCH_QT602240)
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
...
#if defined(CONFIG_MARIMBA_CORE)	&& !defined(CONFIG_KTTECH_WIFI_BCM4329)
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
		MSM_GSBI7_QUP_I2C_BUS_ID,
...
{
#ifndef CONFIG_MACH_KTTECH // Surf Board Only
#if !defined(CONFIG_USB_PEHCI_HCD) && !defined(CONFIG_USB_PEHCI_HCD_MODULE)
...
static void __init msm8x60_init_buses(void)
{
	...
	iounmap(gsbi_mem);
#if defined(CONFIG_KTTECH_TOUCH_QT602240) || defined(CONFIG_KTTECH_TOUCH_MCS7000) || defined(CONFIG_KTTECH_TOUCH_QT602240_V3)
  	msm_gsbi1_qup_i2c_device.dev.platform_data = &msm_gsbi1_qup_i2c_pdata; //KT tech. Touch driver
#endif // defined(CONFIG_KTTECH_TOUCH_QT602240) ||defined(CONFIG_KTTECH_TOUCH_MCS7000) 	
#ifndef CONFIG_KTTECH_TDMB_SERVICE
...
#endif
#ifdef CONFIG_KTTECH_WIFI_BCM4329 // BCM4329(WI-FI/BT) REG_ON/WIFI_RST_N/BT_RST_N
...
#endif
#ifdef CONFIG_MACH_KTTECH  //sleep current
    // boot config
...
#ifdef CONFIG_FB_MSM_MIPI_DSI
#define KTTECH_GPIO_LCD_RESET_N   (102)
#define MDP_VSYNC_GPIO 28
#define MIPI_BLOCK_CTRL 1
...
static int mipi_dsi_sharp_qhd_power(int on){
	int rc = 0;
	int flag_on = !!on;
	static int mipi_dsi_power_save_on;
#if MIPI_BLOCK_CTRL
	static struct regulator *mipi_reg_l16;			/* MIPI VDD */
#endif
...
retry:
#if MIPI_BLOCK_CTRL
	if (mipi_reg_l16 == NULL)
		_GET_REGULATOR(mipi_reg_l16, "8058_l16");	

	if (mipi_reg_l16 == NULL) {
		printk(KERN_ERR "%s: Regulator Get (8058_L16, 1.8v) Failed!\n", __func__);
		return 0;
	}
#endif
	if (display_reg == NULL)
	...
	if (display_reg == NULL) {
		printk(KERN_ERR "%s: Regulator Get (8902_L2, 3.0v) Failed!\n", __func__);
		return 0; 
	}

	rc = gpio_tlmm_config(GPIO_CFG(KTTECH_GPIO_LCD_RESET_N, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);

	if (rc) {
		printk(KERN_ERR "%s: LCD_RESET_N gpio %d request or tlmm_config"
			"failed\n", __func__,
			KTTECH_GPIO_LCD_RESET_N);
		goto out;
		}

	if (flag_on) {
#if MIPI_BLOCK_CTRL
		rc = regulator_set_voltage(mipi_reg_l16, 1800000, 1800000);
		if (rc) {
			printk(KERN_ERR "%s: Regulator Set Voltave (8058_L16, 1.8v) Failed!\n", __func__);
			goto out;
		}

		rc = regulator_enable(mipi_reg_l16);
		if (rc) {
			printk(KERN_ERR "%s: Regulator Enable (8058_L16, 1.8v) Failed!\n", __func__);
			goto out;
		}
#endif
		rc = regulator_set_optimum_mode(display_reg,100000); // For Test & Apply.
		if (rc < 0) {
				printk(KERN_ERR "%s: Regulator Set Optimum Mode Failed - ON (8901_L2, 3.0v) Failed!\n"
							, __func__);	
				goto out;
		}		
		...
		if (rc) {
			printk(KERN_ERR "%s: Regulator Set Voltave (8901_L2, 3.0v) Failed!\n", __func__);
			goto out;
		}

		rc = regulator_enable(display_reg);
		if (rc) {
			printk(KERN_ERR "%s: Regulator Enable (8901_L2, 3.0v) Failed!\n", __func__);	
			goto out;
		}
		/* LCD Reset Sequence */
		gpio_set_value(KTTECH_GPIO_LCD_RESET_N, 1);
		msleep(1);
		gpio_set_value(KTTECH_GPIO_LCD_RESET_N, 0);
		msleep(1);
		gpio_set_value(KTTECH_GPIO_LCD_RESET_N, 1);
		msleep(50);
	} 
			...
			if (rc) {
			printk(KERN_ERR "%s: Regulator Disable (8901_L2, 3.0v) Failed!\n", __func__);
				goto out;
			}
			msleep(1);
#if MIPI_BLOCK_CTRL
			rc = regulator_disable(mipi_reg_l16);
			if (rc) {
			printk(KERN_ERR "%s: Regulator Disable (8058_L16, 1.8v) Failed!\n", __func__);
				goto out;
			}
			msleep(1);
#endif
...
out:
	retry_val++;
	printk(KERN_ERR "%s: A Problem Occured! on MIPI-DSI Power Control. Need to Re-initialize DSI!, Retry : %d\n", __func__, retry_val);
#if MIPI_BLOCK_CTRL
	regulator_put(mipi_reg_l16);
	mipi_reg_l16 = NULL;
#endif
	regulator_put(display_reg);
	display_reg = NULL;		
	...
	if(retry_val <5) 
		...
		printk(KERN_ERR "%s: A Problem Occured! on MIPI-DSI Power Control. Re-initialize DSI!, Failed!\n", __func__);
		return rc;
	}
}
...
#ifdef CONFIG_KTTECH_WIFI_BCM4329
#define KTTECH_GPIO_BCM4329_REG_ON   (51)
#define KTTECH_GPIO_BCM4329_WOW      (50)
...
static struct pm8058_gpio pmic_sleep_clk_gpio = {
	.direction      = PM_GPIO_DIR_OUT,
	.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
	.output_value   = 1,
	.pull           = PM_GPIO_PULL_NO,
...
static struct pm8058_gpio pmic_sleep_clk_off_gpio = {
	.direction      = 0x0,
	.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
	.output_value   = 1,
	.pull           = PM_GPIO_PULL_NO,
...
*
******************************************************************************/
void *bcm4329_wifi_mem_prealloc(int section, unsigned long size)
{
...
static int __init bcm4329_init_wifi_mem (void)
{
	int i;
    ...
    for(i=0;( i < PREALLOC_WLAN_NUMBER_OF_SECTIONS );i++)
    {
        ...
        {
			printk(" bcm4329_init_wifi_mem ERROR \n");        
            return -ENOMEM;
        }
    }
...
EXPORT_SYMBOL(bcm4329_wifi_mem_prealloc);

#endif 
/******************************************************************************
* FUNCTION
*   bcm4329_pwr_control(int on)
*
* DESCRIPTION   
*   BCM4329���� �����ϴ� VDDIO On. 
*    
...
static int bcm4329_pwr_control(int on)
{
	int rc = 0;
   ...
   msm_sdcc_setup_pad(4, on);
	...
	if (on) {
        ...
        rc = pm8058_gpio_config(PMIC_GPIO_SLEEP_CLK,
        	&pmic_sleep_clk_gpio);
        if (rc) {
			printk("clk error  bcm4329_pwr_control   \n");			
		...
		rc = regulator_set_voltage(vreg_wlan_1_8v,
				1800000, 1800000);
		if (rc) {
			printk("error   bcm4329_pwr_control   ");						
...
/******************************************************************************
* FUNCTION
*   bcm4329_ldo_power(int)
*
* DESCRIPTION   
*   BCM4329 LDO(bt, wifi  ) control
*    
* PARAMETERS    
*
* RETURN VALUE  
*
******************************************************************************/
#define bcm4329_wifi 0x01
#define bcm4329_bluetooth 0x02

int bcm4329_device = 0;
int bcm4329_powered = 0;

void bcm4329_set_ldo_pwr_state(int power)
{
  bcm4329_powered = power;
}

int bcm4329_get_ldo_pwr_state(void)
{
  return bcm4329_powered;
}

int bcm4329_ldo_pwr(int device, int on)
{
    if(on)
        bcm4329_device |= device;
    else
        bcm4329_device &=~ device;

	gpio_tlmm_config(GPIO_CFG(KTTECH_GPIO_BCM4329_REG_ON, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);


    if((bcm4329_device & bcm4329_wifi) || (bcm4329_device & bcm4329_bluetooth))
    {
        if(!bcm4329_get_ldo_pwr_state())
        {
            bcm4329_pwr_control(on);
			msleep(60);
            gpio_set_value(KTTECH_GPIO_BCM4329_REG_ON, on);
			msleep(1);
			gpio_set_value(KTTECH_GPIO_BCM4329_REG_ON, 0);
			msleep(1);
			gpio_set_value(KTTECH_GPIO_BCM4329_REG_ON, on); 
            bcm4329_set_ldo_pwr_state(on);
        }
        else
        {
            printk("   %s() : Already Power ON, device = %x, bcm4329_powered = %d\n", __func__, bcm4329_device, bcm4329_get_ldo_pwr_state());
        }
    }
    else
    {
...
EXPORT_SYMBOL(bcm4329_ldo_pwr);
EXPORT_SYMBOL(bcm4329_get_ldo_pwr_state);
...
EXPORT_SYMBOL(bcm4329_wifi_reset);
...
int bcm4329_wifi_wow_en(int on)
{
	
#if 1
    if(on)
    {
      gpio_tlmm_config(GPIO_CFG(KTTECH_GPIO_BCM4329_WOW, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    }
    else
    {
      gpio_tlmm_config(GPIO_CFG(KTTECH_GPIO_BCM4329_WOW, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    }
#endif
    return 0;
...
EXPORT_SYMBOL(bcm4329_wifi_wow_en);
...
#else //CONFIG_KTTECH_WIFI_BCM4329
static int bluetooth_power(int on)
{
    int rc = 0;
    ...
    bcm4329_ldo_pwr(bcm4329_bluetooth, on);   
    bcm4329_bt_reset(on);        
...
static void __init msm8x60_init(struct msm_board_data *board_data)
{
    ...
    {
		usb_products[1].num_functions	= ARRAY_SIZE(usb_functions_default_adb_ftm);
		usb_products[1].functions	= usb_functions_default_adb_ftm;
    }
#endif
	/*
...
#ifdef CONFIG_BATTERY_MSM8X60
#ifdef CONFIG_MACH_KTTECH
    // Enable Charging on FTM if(!get_kttech_ftm_mode())
