...
static int voice_create_mvm_cvs_session(struct voice_data *v)
{
	...
	if (!cvs_handle) {
		...
		pr_debug("send stream create session pkt size = %d\n",
...
static int voice_config_cvs_vocoder(struct voice_data *v)
{
	...
	case VSS_MEDIA_ID_EVRC_MODEM: {
		...
		pr_debug("%s: Setting EVRC min-max rate\n", __func__);
	...
	case VSS_MEDIA_ID_AMR_WB_MODEM: {
		...
		pr_debug("%s: Setting AMR WB rate\n", __func__);
			...
			void *private_data)
{
	...
	pr_debug("auddev_cb_function, evt_id=%d,\n", evt_id);
		...
		if (data->payload_size) {
			...
			if (ptr[0] ==
				...
				if (!ptr[1]) {
					...
					pr_debug("got NACK from CVP create \
						session response\n");
				v->cvp_state = CMD_STATUS_SUCCESS;
				wake_up(&v->cvp_wait);
			} else if (ptr[0] ==
