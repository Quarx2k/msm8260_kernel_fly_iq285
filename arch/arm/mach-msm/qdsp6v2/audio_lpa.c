...
static long audio_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case AUDIO_START:
		pr_debug("%s: AUDIO_START: Session %d\n", __func__,
			audio->ac->session);
		if (!audio->opened) {
	...
	case AUDIO_STOP:
		pr_debug("%s: AUDIO_STOP: session_id:%d\n", __func__,
			audio->ac->session);
		audio->stopped = 1;
...
static int audio_release(struct inode *inode, struct file *file)
{
	...
	pr_debug("%s: audio instance 0x%08x freeing, session %d\n", __func__,
...
static int audio_open(struct inode *inode, struct file *file)
{
	...
	pr_debug("%s: audio instance 0x%08x created session[%d]\n", __func__,
						(int)audio,
						audio->ac->session);
done:
	return rc;
