				...
				unsigned long arg)
{
	...
	case AUDIO_STOP:
		...
		if (atomic_read(&pcm->amp_stopped) == 0) {
#if defined(CONFIG_KTTECH_SOUND_WM8993)
#elif defined(CONFIG_KTTECH_SND_CODEC_WM8993)
	...
	case AUDIO_FLUSH:
		...
		if (atomic_read(&pcm->amp_stopped) == 1) {
#if defined(CONFIG_KTTECH_SOUND_WM8993)
#elif defined(CONFIG_KTTECH_SND_CODEC_WM8993)
