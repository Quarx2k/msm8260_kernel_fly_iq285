...
struct wake_lock kttech_snd_wakelock;
#endif
...
#if !defined(CONFIG_KTTECH_SND_CODEC_WM8993) && !defined(CONFIG_KTTECH_SOUND_WM8993)
...
#ifdef CONFIG_MACH_KTTECH
   wake_lock_timeout(&kttech_snd_wakelock, 10*HZ);
#endif
	...
	if (msm_codec_i2s_slave_mode) {
		pr_debug("%s: configuring ws for slave mode\n", __func__);
...
#ifdef CONFIG_MACH_KTTECH
   wake_lock_timeout(&kttech_snd_wakelock, 3*HZ);
#endif
...
static int snddev_icodec_check_freq(u32 req_freq)
{
	...
	if ((req_freq != 0) && (req_freq >= 8000) && (req_freq <= 48000)) {
		if ((req_freq == 8000) || (req_freq == 11025) ||
			...
			pr_debug("%s: Unsupported Frequency:%d\n", __func__,
								req_freq);
	}
	return rc;
}
		...
		u32 volume)
{
	...
	if (!dev_info) {
		pr_debug("%s : device not intilized.\n", __func__);
		return  -EINVAL;
	}
	...
	if (!(icodec->data->dev_vol_type & (SNDDEV_DEV_VOL_DIGITAL
		...
		pr_debug("%s : device %s does not support device volume "
...
static int __init snddev_icodec_init(void)
{
	...
	wake_lock_init(&icodec_drv->rx_idlelock, WAKE_LOCK_IDLE,
			"snddev_rx_idle");
#ifdef CONFIG_MACH_KTTECH
	wake_lock_init(&kttech_snd_wakelock, WAKE_LOCK_SUSPEND, "kttech_snd");
#endif
...
#ifdef CONFIG_MACH_KTTECH
    wake_lock_destroy(&kttech_snd_wakelock);
#endif
	platform_driver_unregister(&snddev_icodec_driver);
