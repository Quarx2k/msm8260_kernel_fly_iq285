...
#if 0//defined(CONFIG_MACH_KTTECH) && !defined(CONFIG_MSM_DLOAD_MODE)
static void set_dload_mode_clear(void )
{
	void *dload_mode_addr;
	dload_mode_addr = ioremap_nocache(0x2A05F000, SZ_4K);
	if (dload_mode_addr) {
...
void arch_reset(char mode, const char *cmd)
{
	...
	*/
	...
	if (cmd != NULL) {
      ...
      {
        writel(0x6f656d00 | get_kttech_ftm_mode(), restart_reason);
      }
