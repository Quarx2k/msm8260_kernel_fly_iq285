...
#ifdef CONFIG_MACH_KTTECH
static int g_product_id;
static int g_pre_product_id;
...
static int get_product_id(struct android_dev *dev)
{
   ...
   if(p->product_id == 0x3502 || p->product_id == 0x3500)
    return 0x3500;
   
   if(dev->product_id != g_product_id)
   {
      return g_product_id;
   }
#endif
...
#ifdef CONFIG_MACH_KTTECH
static void kttech_config_functions(const char *f_name, int enable)
{
	struct android_dev *dev = _android_dev;
	struct android_usb_product *up = dev->products;
	int index;
...
void android_enable_function(struct usb_function *f, int enable)
{
	...
	if (!!f->disabled != disable) {
		...
		if (!strcmp(f->name, "rndis")) {
...
#ifdef CONFIG_MACH_KTTECH
				g_product_id = g_pre_product_id;
#endif
			}
#ifdef CONFIG_MACH_KTTECH
          if(g_product_id != 0x3500)
          {
            kttech_config_functions("usb_mass_storage", (g_product_id==0x1000));
          }
          else if(g_product_id == 0x3500)
          {
            kttech_config_functions("rndis", enable);
          }
          else
#endif
          ...
          if (!strcmp(f->name, "adb") && (g_product_id != 0x3500))
          {
              if(enable)
                g_product_id = 0x3403;
              else
                g_product_id = 0x1000;
              kttech_config_functions("usb_mass_storage", !enable);
              g_pre_product_id=g_product_id;
          }
		}
#endif
...
#ifdef CONFIG_MACH_KTTECH
       printk(KERN_CRIT "android_enable_function : pid is %x", product_id);
#endif
	}
}
