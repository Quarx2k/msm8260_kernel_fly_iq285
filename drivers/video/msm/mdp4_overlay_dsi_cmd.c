...
#ifdef CONFIG_MACH_KTTECH
		(mdp4_overlay_is_state() == FALSE || pipe->pipe_num == OVERLAY_PIPE_VG1 || cam_get_power() == TRUE) &&
#endif
...
void mdp4_dsi_cmd_dma_busy_wait(struct msm_fb_data_type *mfd)
{
	...
	if (need_wait) {
		...
		if (need_wait > 2){
			DEBUG_KTTECH_INFO("%s: pending pid=%d dsi_clk_on=%d\n",
					__func__, current->pid, mipi_dsi_clk_on);
		}
