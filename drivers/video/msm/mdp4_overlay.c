...
static int mdp4_overlay_set_state = FALSE;
#endif
...
#ifdef CONFIG_MACH_KTTECH
int mdp4_overlay_is_state(void)
{
	return mdp4_overlay_set_state;
}
#endif
...
int mdp4_overlay_set(struct fb_info *info, struct mdp_overlay *req)
{
...
#ifdef CONFIG_FB_MSM_MIPI_DSI
#ifdef CONFIG_MACH_KTTECH
	/* KT Tech : Too many overlay set - Video Player, MX Player, pooq(iMBC) ... */
	if (old_perf_level == new_perf_level) {
		skip_perf_level = TRUE;
	} else {
		pr_info("%s: src_format %u, src_w %u, src_h %u, dst_w %u, dst_h %u\n",
			__func__, pipe->src_format, pipe->src_w, pipe->src_h, pipe->dst_w, pipe->dst_h);

		skip_perf_level = FALSE;
	}

	if (pipe->pipe_num == OVERLAY_PIPE_VG1)
		mdp4_overlay_set_state = TRUE;
#endif
	 ...
	 */
#ifdef CONFIG_MACH_KTTECH
	if (ctrl->panel_mode & MDP4_PANEL_DSI_CMD && mfd->panel_power_on)
#else
	if (ctrl->panel_mode & MDP4_PANEL_DSI_CMD)
#endif
	{
...
#ifdef CONFIG_MACH_KTTECH
		} else if (ctrl->panel_mode & MDP4_PANEL_DSI_CMD && mfd->panel_power_on && !skip_perf_level) {
#else
...
#ifdef CONFIG_MSM_BUS_SCALING
#ifdef CONFIG_MACH_KTTECH
	if (pipe->mixer_num == MDP4_MIXER0 && !skip_perf_level)
#else
	if (pipe->mixer_num == MDP4_MIXER0)
#endif
	{
