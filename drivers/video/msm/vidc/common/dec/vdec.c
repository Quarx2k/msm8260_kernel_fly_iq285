...
#ifdef CONFIG_MACH_KTTECH
	if (frame_rate->fps_numerator == 1000000) 
		vcd_frame_rate.fps_denominator = (frame_rate->fps_denominator > MAX_SUPPORTED_FPS) ?
						DEFAULT_FPS :
...
static int vid_dec_open(struct inode *inode, struct file *file)
{
	...
	if (!vid_dec_device_p->num_clients) {
		if (!vidc_load_firmware()) {
			mutex_unlock(&vid_dec_device_p->lock); // 20111018 by ssgun - qc bug
			return -ENODEV;
		}
	}
