...
*/
static char led_pwm3[2] = {0x55, 0x02}; /* DTYPE_DCS_WRITE1 */
...
#ifdef CONFIG_KTTECH_NOVATEK_BACKLIGHT
#define PWM_MAX 255 //255 = 100% duty cycle
#define UI_STEP 28
#define DEFAULT_BL 16

static int boot_delay_bl_on(struct work_struct *w)
{
    ...
    if(fb_backlight_level > 0)
    {
        ...
        led_pwm1[1]  = fb_backlight_level*(PWM_MAX/UI_STEP);
        mipi_dsi_cmds_tx(mfd, &novatek_tx_buf, novatek_cmd_backlight_cmds,
        ARRAY_SIZE(novatek_cmd_backlight_cmds));
    }
    else
    {
        printk("!!!boot_delay_bl_on \n");
        led_pwm1[1] = DEFAULT_BL*(PWM_MAX/UI_STEP);
        mipi_dsi_cmds_tx(mfd, &novatek_tx_buf, novatek_cmd_backlight_cmds,
        ARRAY_SIZE(novatek_cmd_backlight_cmds));
    }
...
static int mipi_novatek_lcd_on(struct platform_device *pdev)
{
	...
	DEBUG_KTTECH_INFO("MSM-FB Func(+) : %s\n", __func__);
...
#if 0//defined(CONFIG_KTTECH_NOVATEK_BACKLIGHT)
		cancel_delayed_work(&mfd->boot_bl_on);
		schedule_delayed_work(&mfd->boot_bl_on,	msecs_to_jiffies(32));
#endif
...
static void mipi_novatek_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	printk("!!!mipi_novatek_set_backlight : %d\n", mfd->bl_level);

	mutex_lock(&mfd->dma->ov_mutex);
    
    pwm_step = mfd->bl_level*(PWM_MAX/UI_STEP);
...
#else
static void mipi_tps1161_set_backlight(struct msm_fb_data_type *mfd)
{
	static int bl_level_old;
...
#ifndef FAST_FB_BL_BLOCK      
    if(fb_backlight_level == mfd->bl_level)
    {
        bl_level_old = mfd->bl_level;
        return;
    }
...
static void mipi_tps1161_bkl_ctrl(struct msm_fb_data_type *mfd, bool on)
{
#ifndef FAST_FB_BL_BLOCK      
    if (!mfd)
      return;
    ...
    {
        printk("!!!fast backlight on : %d \n", fb_backlight_level);
        mfd->bl_level = fb_backlight_level;
        tps61161_set_bl(mfd);
    }
#endif    
}
...
#else
    .set_backlight	= mipi_tps1161_set_backlight, 
	.pre_bl_conrol	= mipi_tps1161_bkl_ctrl,
#endif
};
