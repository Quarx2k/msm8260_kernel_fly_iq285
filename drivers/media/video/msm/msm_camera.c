...
#define KTTECH_CAMERA_MSG(fmt, args...) printk(KERN_INFO "msm_camera: " fmt, ##args)  // jykim
//#undef CDBG
//#define CDBG(fmt, args...) printk(KERN_INFO "msm_camera: " fmt, ##args)  // jykim

#ifdef CONFIG_MSM_CAMERA_DEBUG // jykim
...
//#if defined(CONFIG_KTTECH_MT9M113) || defined(CONFIG_KTTECH_MT9E013)
	KTTECH_CAMERA_MSG("%s: type %d, paddr 0x%lx, vaddr 0x%lx\n",
...
#if 0
//#if defined(CONFIG_KTTECH_MT9M113) || defined(CONFIG_KTTECH_MT9E013)
	KTTECH_CAMERA_MSG("%s: y %x, cbcr %x, qcmd %x, virt_addr %x\n",
		__func__,
...
static int msm_pp_release(struct msm_sync *sync, void __user *arg)
{
	...
	if (!sync->pp_mask) {
		...
		pr_debug("%s: pp not in progress for\n", __func__);
#else
	...
	unsigned long arg)
{
	...
	case MSM_CAM_IOCTL_FLASH_CTRL: {
		...
		#ifdef CONFIG_MSM_CAMERA_FLASH // KT Tech
