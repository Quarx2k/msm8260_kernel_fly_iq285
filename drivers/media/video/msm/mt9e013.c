...
#ifdef CONFIG_KTTECH_MT9E013 // jykim 110322
	...
	unsigned short *rdata, int rlen)
{
	...
	buf[1] = (raddr & 0x00FF);
	rc = mt9e013_i2c_rxdata(mt9e013_client->addr, buf, rlen);
	if (rc < 0) {
...
#endif
	rc = mt9e013_i2c_txdata(mt9e013_client->addr, buf, 4);
	if (rc < 0) {
...
static int32_t mt9e013_i2c_write_b_sensor(unsigned short waddr, uint8_t bdata)
{
	...
	CDBG("i2c_write_b addr = 0x%x, val = 0x%x\n", waddr, bdata);
#endif
	rc = mt9e013_i2c_txdata(mt9e013_client->addr, buf, 3);
	if (rc < 0) {
		CDBG("i2c_write_b failed, addr = 0x%x, val = 0x%x!\n",
			waddr, bdata);
	}
...
#ifdef CONFIG_KTTECH_MT9E013 // Added by hwanga for loading Calibration Data - 2011.03.30
...
static int32_t mt9e013_sensor_setting(int update_type, int rt)
{
	...
	if (update_type == REG_INIT) {
		...
		if (!CSI_CONFIG) {
#ifndef CONFIG_KTTECH_MT9E013 // jykim : undefined symbol
...
{
#ifdef CONFIG_KTTECH_MT9E013 // jykim
	CDBG("%s: e\n", __func__);
	...
	const struct i2c_device_id *id)
{
	...
	mt9e013_client->addr = mt9e013_client->addr >> 1;
...
static int mt9e013_sensor_release(void)
{
	...
	mutex_lock(&mt9e013_mut);
	mt9e013_power_down();
#ifndef CONFIG_KTTECH_MT9E013 // jykim remove warning
