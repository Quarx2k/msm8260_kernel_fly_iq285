...
#if defined(CONFIG_KTTECH_TOUCH_QT602240)
extern unsigned int qt602240_get_chg_state;
#elif defined(CONFIG_KTTECH_TOUCH_QT602240_V3)
#define CHG_MODE_AC	1
#define CHG_MODE_VBATT	0
extern void o3_inform_charger_connection(int mode);
#endif
int last_battery_soc = 0; /*마�?막으�??�어??soc �?*/ 
#endif
...
#ifdef CONFIG_KTTECH_BATTERY
		val->intval = get_battery_mvolts() * 1000;
#else
...
#ifdef CONFIG_HAS_WAKELOCK
#ifdef CONFIG_KTTECH_BATTERY
	wake_lock_init(&priv->wl, WAKE_LOCK_SUSPEND, "pm8058-usb");
#else
...
#ifdef CONFIG_KTTECH_BATTERY
	{
		int current_soc = msm_chg.get_batt_capacity_percent();
		if(last_battery_soc != current_soc)
		{
			last_battery_soc = current_soc;
			power_supply_changed(&msm_psy_batt);
		}
	}
#endif
