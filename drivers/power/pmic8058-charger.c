...
static int pm_chg_kttech_factmode_set(int value)
{
    ...
    pm8058_read(pm8058_chg.pm_chip, PM8058_CHG_TRICKLE, &old, 1);

	data = (uint8_t)((3200-1800)/100) ;
	temp = (old &0x0F) | (data<<4);

    pm8058_write(pm8058_chg.pm_chip, PM8058_CHG_TRICKLE, &temp, 1);
   ...
   if(ret)
    ...
    pm8058_write(pm8058_chg.pm_chip, PM8058_CHG_CNTRL, &temp, 1);
...
#define O3_BATT_ID_MIN_MV  10
	enum O3_HW_VER hw_ver = get_kttech_hw_version();
	...
	if( (hw_ver == O3_PROTO) || (hw_ver == O3_ES1))
	{
		return 1;
	}
	else
...
static int pm8058_get_battery_mvolts(void)
{
	...
	//printk(KERN_INFO "%s: vbatt_mv is %d\n", __func__, vbatt_mv);
...
static int pm8058_get_battery_soc(void)
{
	...
	if(vbatt_mv > 0) {
		if((batt_soc == 0) && (vbatt_mv > 3500)) {
			/*printk(KERN_INFO "kttech-charger soc [%d] [%d]mv\n", batt_soc, vbatt_mv); */
			batt_soc = 1;
