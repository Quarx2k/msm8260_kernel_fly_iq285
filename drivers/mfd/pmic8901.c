...
int pm8901_rev(struct pm8901_chip *chip)
{
	if (chip == NULL) {
		if (pmic_chip != NULL)
			...
			return 0xF4;  // KT Tech Initial PMIC Revision
#else
			return -EINVAL;
#endif
	}
...
#ifdef CONFIG_MACH_KTTECH
	/*by IMKIM (KT tech)
	If PM8901 have problem of unknown reason then force reboot our
	system for prevent to S4B dead. */
	printk("PM8901 HARDWARE got problem\n");
	panic("PM8901 HARDWARE got problem");
#endif
	spin_lock_irqsave(&chip->pm_lock, irqsave);
