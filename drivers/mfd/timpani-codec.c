	...
	const char __user *ubuf, size_t cnt, loff_t *ppos)
{
	...
	if (!strcmp(access_str, "power")) {
		...
		pr_debug("************** timpani regs *************\n");
		for (i = 0; i < 0xFF; i++) {
			read_result = adie_codec_read(i, &reg_val);
			if (read_result < 0) {
				pr_debug("failed to read codec register\n");
				break;
			} else
...
#if 0 // 20110506 by sssgun //def CONFIG_PM
static int timpani_codec_suspend(struct platform_device *pdev, pm_message_t state)
{
	/* SUSPEND 상태일 경우에만 RESET 를 수행한다. */
#ifdef CONFIG_KTTECH_SND_CODEC_WM8993
	adie_codec.suspend_state = true;
	if(wm8993_get_pathtype() == 0) {
		pr_debug("%s: wm8993_get_pathtype = %d\n", __func__, wm8993_get_pathtype());
		wm8993_reset();
	}
#endif /*CONFIG_KTTECH_SND_CODEC_WM8993*/
	return 0;
...
static int timpani_codec_resume(struct platform_device *pdev)
{
#ifdef CONFIG_KTTECH_SND_CODEC_WM8993
	adie_codec.suspend_state = false;
#endif /*CONFIG_KTTECH_SND_CODEC_WM8993*/
	return 0;
}
#endif
