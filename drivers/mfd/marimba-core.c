...
#ifdef CONFIG_KTTECH_SND_CODEC_WM8993
	if(MARIMBA_SLAVE_ID_MARIMBA != marimba->mod_id)
	{
//pr_debug("==========[[ %s ] ERROR : mod_id=%d, addr=0x%x]==========\n", __func__, marimba->mod_id, marimba->client->addr);
		return EAFNOSUPPORT;
	}
#endif //CONFIG_KTTECH_SND_CODEC_WM8993
...
#if 0 // 20101227 by ssgun
	marimba = &marimba_modules[MARIMBA_SLAVE_ID_CDC];
	marimba->client->addr = 0x34>>1;
#else
	marimba = &marimba_modules[MARIMBA_SLAVE_ID_MARIMBA];
#endif

//printk("==========[[ %s ] addr=0x%x]==========\n", __FUNCTION__, marimba->client->addr);

	value[0] = (reg & 0x00FF);

	msg[0].addr = marimba->client->addr;
	msg[0].flags = 0;
	msg[0].len = 1;
	msg[0].buf = (u8*)value;

	msg[1].addr = marimba->client->addr;
	...
	{
		rc = i2c_transfer(marimba->client->adapter, msg, 2);
		if(rc > 0)
		{
			break;
		}
...
//printk("==========[[ %s ] addr=0x%x]==========\n", __FUNCTION__, marimba->client->addr);
	...
	for (retry = 0; retry <= 2; retry++)
	{
		rc = i2c_transfer(marimba->client->adapter, msg, 1);
		if(rc > 0)
		{
			break;
		}
...
#if 0 //20110506 by ssgun def CONFIG_PM
static int marimba_suspend(struct i2c_client *client, pm_message_t msg)
{
	return 0;
}

static int marimba_resume(struct i2c_client *client)
{
	return 0;
}
#endif
...
#if 0 //20110506 by ssgun def CONFIG_PM
		.suspend = marimba_suspend,
		.resume = marimba_resume,
#endif
};
...
static int __init marimba_init(void)
{
	...
	if (rc) {
		pr_err("%s] Audio gpio  config failed: %d\n", __func__, rc);
		//return rc;
	}
#endif /*CONFIG_KTTECH_SND_CODEC_WM8993*/
