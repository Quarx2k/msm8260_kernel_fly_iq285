...
static void pwrkey_reinit(struct pmic8058_pwrkey *pwrkey)
{
	int err;
	unsigned int delay;
	u8 pon_cntl;
...
static irqreturn_t pwrkey_release_irq(int irq, void *_pwrkey)
{
	...
	if (pwrkey->pressed_first) {
		...
		if (!pwrkey->pdata->pwrkey_time_ms) {
			...
			input_sync(pwrkey->pwr);
			spin_unlock_irqrestore(&pwrkey->lock, flags);
            printk(KERN_CRIT "\n ### BUG ###: POWER Key ==> [RELEASE] \n");                  
            #ifdef CONFIG_MACH_KTTECH
            pwrkey_reinit(pwrkey);
            #endif            
			return IRQ_HANDLED;
		}
		input_report_key(pwrkey->pwr, KEY_END, 1);
...
static int pmic8058_pwrkey_resume(struct device *dev)
{
   ...
   wake_lock_timeout(&kttech_pwrkey_wakelock, 3*HZ);
...
static int __devinit pmic8058_pwrkey_probe(struct platform_device *pdev)
{
   ...
   set_irq_wake(key_press_irq, 1);
   set_irq_wake(key_release_irq, 1);
#endif
