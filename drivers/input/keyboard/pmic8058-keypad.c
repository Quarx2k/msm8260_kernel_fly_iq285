...
struct work_struct kttech_key_work;
struct wake_lock kttech_key_wakelock;
#endif
...
#ifdef CONFIG_MACH_KTTECH
extern int pm8058_is_pwr_key_pressed(void);

/* 주의 1,2 번중에 선택하여 사용*/
/* 1. volume down 으로 강제로 reset : 시간 조절 가능*/
#define VD_FORCE_RESET
/* 2. volume down 으로 강제로 reset : 시간 조절 불가능 : 10 sec + reset ready time ( 8 sec)*/
//#define VD_STUCK_FORCE_RESET
#endif

#ifdef VD_FORCE_RESET
#define RESET_VOLUMEDOWN 114
#define RESET_DETECT_NUM 40     //100ms* 40 = 4 sec + reset ready time ( 8 sec) = 12 sec
#define DEBUG_RESET_VD 0
struct hrtimer reset_timer;
static int check_reset_count;
#endif

#ifdef VD_STUCK_FORCE_RESET
static int check_stuck_key;
#endif

#ifdef VD_FORCE_RESET
static enum hrtimer_restart reset_timer_function(struct hrtimer *timer)
{
    //check routine
    check_reset_count ++;
#ifdef DEBUG_RESET_VD    
    printk("#### RESET timer funtion 500ms = %d, power key = %d ###\n", check_reset_count, pm8058_is_pwr_key_pressed());
#endif
    if(pm8058_is_pwr_key_pressed())
    {
        if(check_reset_count > RESET_DETECT_NUM)
        {
          check_reset_count = 0;
          hrtimer_cancel(&reset_timer);          
          panic(" USER H/W RESET ");
          return HRTIMER_NORESTART;          
        }  
          
    	hrtimer_start(&reset_timer,
          ktime_set(0,    
          100000000), // 100ms
          HRTIMER_MODE_REL);
   }  
					 ...
					 u16 *old_state)
{
	...
	for (row = 0; row < kp->pdata->num_rows; row++) {
		...
		for (col = 0; col < kp->pdata->num_cols; col++) {
...
#if defined(VD_FORCE_RESET)
           if((kp->keycodes[code] == RESET_VOLUMEDOWN) && (!(new_state[row] & (1 << col)) == 1)) //press vol down key
           {
#ifdef DEBUG_RESET_VD    
                printk(" ***** volume down [press] *****n");
#endif
          		hrtimer_start(&reset_timer,
          				ktime_set(0,    
          				100000000), // 100ms
           ...
           else if((kp->keycodes[code] == RESET_VOLUMEDOWN) && (!(new_state[row] & (1 << col)) == 0)) //release vol down key
           {
#ifdef DEBUG_RESET_VD    
                printk(" ***** volume down [release] *****\n");
#endif
                check_reset_count = 0;
        		hrtimer_cancel(&reset_timer);
           }
#elif defined(VD_STUCK_FORCE_RESET)
            if((kp->keycodes[code] == RESET_VOLUMEDOWN) && (!(new_state[row] & (1 << col)) == 1) && check_stuck_key == 1) //press stuck vol down key
            {
                check_stuck_key = 0;
                if(pm8058_is_pwr_key_pressed())
                {
...
#ifdef CONFIG_MACH_KTTECH
static void kttech_key_work_f(struct work_struct *work)
{
   wake_lock_timeout(&kttech_key_wakelock, 2*HZ);
}
#endif
/*
...
#ifdef CONFIG_MACH_KTTECH
	if (!work_pending(&kttech_key_work))
		schedule_work(&kttech_key_work);    
#endif
...
#ifdef CONFIG_MACH_KTTECH
    wake_lock_init(&kttech_key_wakelock, WAKE_LOCK_SUSPEND, "kttech_key");
    INIT_WORK(&kttech_key_work, kttech_key_work_f);
#endif

#ifdef VD_FORCE_RESET
	hrtimer_init(&reset_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	reset_timer.function = reset_timer_function;
#endif
...
static int __devexit pmic8058_kp_remove(struct platform_device *pdev)
{
	...
	free_irq(kp->key_sense_irq, NULL);
#ifdef CONFIG_MACH_KTTECH
   cancel_work_sync(&kttech_key_work);
   wake_lock_destroy(&kttech_key_wakelock);
#endif
	input_unregister_device(kp->input);
